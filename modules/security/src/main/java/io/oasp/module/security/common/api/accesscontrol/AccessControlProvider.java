 ...
 * {@link #collectAccessControlIds(String, Set) collect} all {@link AbstractAccessControl}s for an ID of a
 * {@link AbstractAccessControl} (typically a {@link AccessControlGroup} or role). This is used to expand the groups
 * provided by the access-manager (authentication and identity-management) to the full set of
 * {@link AccessControlPermission permissions} of the {@link java.security.Principal user}.<br/>
...
public interface AccessControlProvider {
   ...
   * precisely of all {@link AbstractAccessControl}s) contained in the {@link AbstractAccessControl}
   ...
   *        {@link AccessControlGroup}) to collect.
   * @param permissions is the {@link Set} where to {@link Set#add(Object) add} the collected
   *        {@link AbstractAccessControl}s. This will include the {@link AbstractAccessControl}
   *        {@link AbstractAccessControl#getId() identified} by the given <code>groupId</code>.
   * @return {@code true} if the given <code>groupId</code> has been found, {@code false} otherwise.
   */
